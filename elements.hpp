...
class Block {
  ...
  public:
    ...
    Block(const std::string &blk_, const std::string &id_,
          const std::string &kit_, const std::string &name_) {
	setBlk(blk_);
	setID(id_);
	setKit(kit_);
	setName(name_);
    }
    Block(const std::string &blk_, const unsigned short back,
          const std::string &id_, const std::string &kit_,
	...
	if (back > 255) {
	    ...
	    oss << back;
	    blk = "\033[48;5;" + oss.str() + "m" +
	          (blk_ == "null" ? "  " : blk_) + "\033[0m";
	}
    }
...
class LargeBlock {
  ...
  public:
    ...
    LargeBlock(const size_t w, const size_t h, const std::string &id_,
               const std::string &kit_, const std::string &name_) {
	setW(w);
	setH(h);
	setID(id_);
    ...
    LargeBlock(const json &j) { fromJson(j); }
    LargeBlock(const json &j, const std::string &kit_) {
	fromJson(j);
	setKit(kit_);
    }

    std::vector<std::vector<Block>> getLblk() const & { return lblk; }
    void setLblk(const std::vector<std::vector<Block>> &lblk_) { lblk = lblk_; }
    ...
    size_t getW() const { return lblk.empty() ? 0 : lblk[0].size(); }
    ...
    size_t getH() const { return lblk.size(); }
    void setH(const size_t h) { lblk.resize(h, std::vector<Block>(getW())); }
	...
	for (size_t r = 0; r < j.at("blks").size(); r++) {
		...
		setPos(r, c,
		       Block(j.at("blks")[r][c].template get<std::string>()));
	    }
	}
    }
    json toJson() const & {
...
class Kit {
  ...
  public:
    ...
    std::vector<LargeBlock> getLblks() const & { return lblks; }
    void AddToLblks(const LargeBlock &lblk) { lblks.emplace_back(lblk); }
