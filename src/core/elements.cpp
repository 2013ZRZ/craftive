...
void rgb::fromJson(const json &j) {
    if (!j.is_array())
        throw CrtExcept(
            0x0008, translate("rgb", "from rgb::fromJson(); the RGB color in the json object isn't an array"));
    if (j.size() != 3)
        throw CrtExcept(0x0008, translate("rgb", "from rgb::fromJson(); incorrect number of color channels"));
    r = j[0].get<uint8_t>();
    g = j[1].get<uint8_t>();
    b = j[2].get<uint8_t>();
}
...
void Ucc::fromJson(const json &j) {
    if (!j.is_object())
        throw CrtExcept(
            0x0006,
            translate("Ucc", "from Ucc::fromJson(); the unicode colored character's json isn't an object"));
    QString _c = j.at("c").get<QString>();
    if (_c.size() > sizeof(char32_t))
        throw CrtExcept(
            0x0003,
            translate("Ucc", "from Ucc::fromJson(); the string to parse is %1 and it has too many characters"),
            _c);
...
void BasicCrtClass::setID(const QString &_id) {
    if (isInvalidID(_id))
        throw CrtExcept(0x0002,
                        translate("BasicCrtClass", "from BasicCrtClass::setID(); the ID includes invalid character(s)"));
    else if (_id.isEmpty())
...
void BasicElement::setID(const QString &_id) {
    if (isInvalidElemID(_id))
        throw CrtExcept(0x0002,
                        translate("BasicElement", "from BasicElement::setID(); the ID includes invalid character(s) or "
                           "isn't like \"kit/elem\""));
    else if (_id.isEmpty())
...
void Block::fromJson(const json &j) {
    if (!j.is_object())
        throw CrtExcept(0x0006, translate("Block", "from Block::fromJson(); the block's json isn't an object"));
    setBlk(Ucc{j.at("blk")});
    if (j.find("id") != j.end())
...
const Ucc &LBlock::getPos(const size_t r, const size_t c) const {
    if (r >= lblk.size() || c >= (lblk.empty() ? 0 : lblk[r].size()))
        throw CrtExcept(
            0x0005,
            translate("LBlock", "from LBlock::getPos(); the required position is (%1,%2), but it's out of range"),
            r,
            c);
    return lblk[r][c];
}
...
void LBlock::setPos(const size_t r, const size_t c, const Ucc &blk) {
    if (r >= lblk.size() || c >= (lblk.empty() ? 0 : lblk[r].size()))
        throw CrtExcept(
            0x0005,
            translate("LBlock", "from LBlock::setPos(); the required position is (%1,%2), but it's out of range"),
            r,
            c);
    lblk[r][c] = blk;
}
...
size_t LBlock::getW(const size_t r) const {
    if (r < w.size())
        ...
        throw CrtExcept(0x0005,
                        translate("LBlock", "from LBlock::getW(); the required row number is %1, but there's only "
...
void LBlock::fromJson(const json &j) {
    ...
    if (!j.at("lblk").is_array())
        throw CrtExcept(0x0006, translate("LBlock", "from LBlock::fromJson(); the \"lblk\" isn't an array"));
    lblk.resize(j.at("lblk").size());
    w.resize(j.at("lblk").size());
    for (size_t r{}; r < j.at("lblk").size(); r++) {
        if (!j.at("lblk")[r].is_array())
            throw CrtExcept(0x0006, translate("LBlock", "from LBlock::fromJson(); Row %1 isn't an array"), r + 1);
    ...
    if (!j.at("w").is_array())
        throw CrtExcept(0x0006, translate("LBlock", "from LBlock::fromJson(); the \"w\" isn't an array"));
...
void BasicProduct::setAuthor(const QString &_author) {
    if (isInvalidEmail(_author))
        throw CrtExcept(
            0x0007,
            translate("BasicProduct", "from BasicProduct::setAuthor(); the string is \"%1\" and it isn't a valid "
               "email address"),
            _author);
    else
        author = _author;
...
void BasicProduct::fromFile(const QString &path) {
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))
        throw CrtExcept(
            0x0000,
            translate("BasicProduct", "from BasicProduct::fromFile(); failed to open the product file at %1 (%2)"),
            path,
            file.errorString());
    QTextStream in{&file};
    ...
    try {
        ...
        throw CrtExcept(0x0006, translate("BasicProduct", "from BasicProduct::fromFile() (%1)"), e.what());
    }
    file.close();
}
...
void BasicProduct::toFile(const QString &path, const uint8_t indent) {
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
        throw CrtExcept(
            0x0000,
            translate("BasicProduct", "from BasicProduct::toFile(); failed to open the product file at %1 (%2)"),
            path,
            file.errorString());
...
void Kit::operator-=(QString &_id) {
    if (!(blks.remove(_id) || lblks.remove(_id)))
        throw CrtExcept(
            0x0004,
            translate("Kit", "from Kit::operator-=; this kit doesn't contain an element who's ID is \"%1\""),
            _id);
    blks.squeeze();
    lblks.squeeze();
}
...
void Kit::fromJson(const json &j) {
    if (!j.is_object())
        throw CrtExcept(0x0006, translate("Kit", "from Kit::fromJson(); the kit's json isn't an object"));

    if (j.find("id") != j.end())
        setID(j.at("id").get<QString>());
    else
        throw CrtExcept(0x0001, translate("Kit", "from Kit::fromJson(); couldn't find \"id\" in the kit's json"));
    ...
    if (j.find("author") != j.end())
        ...
        throw CrtExcept(
            0x0009,
            translate("Kit", "from Kit::fromJson(); couldn't find \"author\" in the json of Kit %1 (ID: %2)"),
            name,
            id);
    ...
    if (j.find("ver") != j.end())
        ...
        throw CrtExcept(
            0x000D,
            translate("Kit", "from Kit::fromJson(); couldn't find \"ver\" in the json of Kit %1 (ID: %2)"),
            name,
            id);
...
const Ucc Map::operator[](const size_t r, const size_t c) {
    if (r > data.size() || c > data.empty() ? 0 : data[r].size())
        throw CrtExcept(
            0x000E,
            translate("Map", "from Map::operator[]; the required position is (%1,%2), but it's out of range"),
            r,
            c);
...
template <> auto Map::get<0>(const size_t r, const size_t c) {
    if (r > data.size() || c > data.empty() ? 0 : data[r].size())
        throw CrtExcept(
            0x000E,
            translate("Map", "from Map::get<0>(); the required position is (%1,%2), but it's out of range"),
            r,
            c);
...
template <> auto Map::get<1>(const size_t r, const size_t c) {
    if (r > data.size() || c > data.empty() ? 0 : data[r].size())
        throw CrtExcept(
            0x000E,
            translate("Map", "from Map::get<1>(); the required position is (%1,%2), but it's out of range"),
            r,
            c);
    if (std::get<1>(data[r][c]) != nullptr) // is at the upper left corner
...
void Map::fromJson(const json &j) {
    if (!j.is_object())
        throw CrtExcept(0x0006, translate("Map", "from Map::fromJson(); the map's json isn't an object"));

    if (j.find("id") != j.end())
        setID(j.at("id").get<QString>());
    else
        throw CrtExcept(0x0001, translate("Map", "from Map::fromJson(); couldn't find \"id\" in the map's json"));
    ...
    if (j.find("author") != j.end())
        ...
        throw CrtExcept(
            0x0009,
            translate("Map", "from Map::fromJson(); couldn't find \"author\" in the json of Map %1 (ID: %2)"),
            name,
            id);
    ...
    if (j.find("ver") != j.end())
        ...
        throw CrtExcept(
            0x000D,
            translate("Map", "from Map::fromJson(); couldn't find \"ver\" in the json of Map %1 (ID: %2)"),
            name,
            id);
    ...
    if (j.find("data") == j.end())
        throw CrtExcept(
            0x000A,
            translate("Map", "from Map::fromJson(); couldn't find \"data\" in the json of Map %1 (ID: %2)"),
            name,
            id);
    ...
    if (!j["data"].is_array())
        throw CrtExcept(
            0x0006,
            translate("Map", "from Map::fromJson(); \"data\" in the json of Map %1 (ID: %2) isn't an array"),
            name,
            id);
    ...
    for (size_t r{}; r < j["data"].size(); r++) {
        if (!j["data"][r].is_array())
            throw CrtExcept(0x0006,
                            translate("Map", "from Map::fromJson(); Row %1 in \"data\" in the json of Map %2 "
                               "(ID: %3) isn't an array"),
                            r,
                            name,
                            id);
        for (size_t c{}; c < j["data"][r].size(); c++) {
            if (!j["data"][r][c].is_string())
                throw CrtExcept(
                    0x0006,
                    translate("Map", "from Map::fromJson(); (%1,%2)'s ID in \"data\" in the json of Map %3 "
                       "(ID: %4) isn't a string"),
                    r,
                    c,
                    name,
            ...
            if (!CoreStatus::instance().containsKit(targetKitID))
                throw CrtExcept(0x000B,
                                translate("Map", "from Map::fromJson(); couldn't find the kit (ID: %1) where "
            ...
            if (!targetKit.contains(elemFullID))
                throw CrtExcept(0x0004,
                                translate("Map", "from Map::fromJson(); Kit %1 (ID: %2) doesn't contain the "
